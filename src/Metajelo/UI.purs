...
accumulateLocationLoop = labelSig' D.h1' "Location" $
  loopS Opt.empty \locOpt -> D.div_ [] do
    ...
    testLoc <- pure $ injectLocationFieldsOpt locOpt
      ...
      Nothing
      true
    display $ D.div' [D.text $ "Test LocOpt" <> (show testLoc)]  -- FIXME: DEBUG
    ...
    newLocMay <- pure $ injectLocationFields -- TODO: use sequencing to get newLocMay from newLoc
      ...
      -- versioning
    _ <- consoleShow $ "identMay: " <> show (Opt.get (SProxy :: _ "institutionID") locOpt) -- FIXME
    _ <- consoleShow $ "instNameMay: " <> show (Opt.get (SProxy :: _ "institutionName") locOpt) -- FIXME
    _ <- consoleShow $ "instTypeMay: " <> show (Opt.get (SProxy :: _ "institutionType") locOpt) -- FIXME
    _ <- consoleShow $ "sOrgMay: " <> show (Opt.get (SProxy :: _ "superOrganizationName") locOpt) -- FIXME
    _ <- consoleShow $ "icMay: " <> show (Opt.get (SProxy :: _ "institutionContact") locOpt) -- FIXME
    _ <- consoleShow $ "sustainMay: " <> show (Opt.get (SProxy :: _ "institutionSustainability") locOpt) -- FIXME
    _ <- consoleShow $ "polsMay: " <> show (Opt.get (SProxy :: _ "institutionPolicies") locOpt) -- FIXME
